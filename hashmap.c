...
HashMap * createMap(long capacity) {
    ...
    nuevoMapa->buckets = calloc(capacity, sizeof(HashMap));
    nuevoMapa->size = 0;
...
void enlarge(HashMap * map) {
    ...
    Pair ** new_buckets = calloc(map->capacity * 2, sizeof(Pair));
    map->capacity = map->capacity * 2;
    map->buckets = new_buckets;
    map->size = 0;
    for(long pos; pos < map->capacity; pos++){
        if(old_buckets[pos] != NULL){
            insertMap(map, old_buckets[pos]->key, old_buckets[pos]->data);
        }
    }
